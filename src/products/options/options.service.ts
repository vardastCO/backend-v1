...
export class OptionsService {
  ...
  async create(input: CreateOptionInput): Promise<Option> {
    ...
    const parentProducts = await ParentProductEntity.findByIds(
      parentProductIds,
    );
    ...
    const isValid = parentProducts.every(
      parentProduct =>
    ...
    if (!isValid) {
      throw new Error(
        "Invalid input: Products must have the same Category, Uom, and Brand in their parent entities.",
      );
    }
