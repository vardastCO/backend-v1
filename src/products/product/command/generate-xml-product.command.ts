...
export class XmlProductCommand extends CommandRunner {
  ...
  async run(): Promise<void> {
    ...
    for (let i = 0; i < totalProductCount; i += productsPerPage) {
        ...
        const { idsAndNames, count } = await this.findProductIdsAndCount(start, end);
  ...
  async findProductIdsAndCount(start: number, end: number): Promise<{ idsAndNames: {id: number, name: string}[]; count: number }> {
    // console.log('start',start,'end',end)
      const [products, count] = await Product.findAndCount({
      where: { id: Between(start, end) },
    });

    const idsAndNames = products.map(product => ({ id: product.id, name: product.name}));
    ...
    private generateUrls(idsAndNames: {id: number, name: string}[]): string[] {
        return idsAndNames.map(idName => `https://www.vardast.com/product/${idName.id}/${idName.name}`);
    }
