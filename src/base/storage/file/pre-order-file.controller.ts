...
export class PreOrderFileController {
  ...
  async getOrderFiles(
    ...
    let template;
    const templateURL =
      "https://storage.vardast.com/vardast/order/pre-invoice-template.html";
    try {
    ...
    try {
      ...
      const data = {
        ...
        buyerName:
          legal !== null
            ? legal.name_company
            : (await (await offer.preOrder).address).delivery_name,
        buyerNationalId: legal !== null ? legal.national_id : "-",
        buyerAddress:
          address_legal !== null
            ? address_legal?.address
            : (await (await offer.preOrder).address).address,
        items: await items,
  ...
  private injectDataIntoTemplate(template: string, data: any): string {
    ...
    const additionalRowsHTML = [...Array(remainingRows)]
      .map(
        () => `
      <tr>
        <td> <br/> </td>
