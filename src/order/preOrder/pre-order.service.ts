...
export class PreOrderService {
  ...
  async createPreOrder(
    createPreOrderInput: CreatePreOrderInput,
    user: User,
  ): Promise<PreOrder> {
    try {
      ...
      if (createPreOrderInput.cellphone) {
        const userByCellphone = await User.findOneBy({
          cellphone: createPreOrderInput.cellphone,
        });
        if (!userByCellphone) {
  ...
  async update(
    ...
    preOrder.request_date = new Date().toLocaleString("en-US", {
      timeZone: "Asia/Tehran",
    });
    ...
    preOrder.expire_time = updatePreOrderInput.expire_date
      ? this.calculateExpirationDate(
          updatePreOrderInput.expire_date,
        ).toLocaleString("en-US", { timeZone: "Asia/Tehran" })
      : null;
    ...
    const updateCurrentStatusByCommingProps = {
      ...
      [PreOrderStatus.PENDING_PRODUCT]: isHaveAddress
        ? PreOrderStatus.PENDING_PRODUCT
        : PreOrderStatus.PENDING_INFO,
      [PreOrderStatus.PENDING_OFFER]:
        isHaveLines && isHaveAddress
          ? PreOrderStatus.PENDING_OFFER
          : PreOrderStatus.PENDING_PRODUCT,
      [PreOrderStatus.COMPLETED]: isHaveOffers
    ...
    preOrder.status =
      updateCurrentStatusByCommingProps[
        updatePreOrderInput.status ?? preOrder.status
      ];
    ...
    try {
      ...
      const offersPromises = Promise.all([
        OfferOrder.find({
          where: {
            preOrderId: order.id,
            type: TypeOrderOffer.CLIENT,
            status: Not(OrderOfferStatuses.PENDING_INFO),
          },
          relations: ["offerLine"],
          order: {
            offerLine: {
      ...
      order.offers = Array.from(uniqueOffers.values()).sort(
        (a, b) => b.id - a.id,
      );

      const lines = await order.lines;

      const filteredAndSortedLines = lines.filter(
        line => line.deleted_at === null,
      );
  ...
  async publicOrders(
    ...
    const maxNumber =
      typeof number === "number" ? Math.min(Math.max(number, 2), 15) : 2;
        ...
        const [orders, image] = await Promise.all([
          PreOrder.find({
            ...
            where: {
              ...
              status: Not(PreOrderStatus.PENDING_INFO),
            },
            take: maxNumber,
            relations: ["lines"],
            order: {
  ...
  async paginate(
    ...
    if (client) {
      ...
      whereConditions["type"] = isRealUserType
        ? TypeOrder.REAL
        : TypeOrder.LEGAL;
    } else {
      if (projectId) {
    ...
    if (
      ...
      if (projectName) {
        whereConditions["project"] = [{ name: Like(`%${projectName}%`) }];
      }
      if (seller) {
    ...
    if (client) {
      await Promise.all(
        data.map(async pre_order => {
          pre_order.lineDetail = (await pre_order.lines)
            .map(line => line.item_name + " - ")
            .join("")
            .slice(0, -3);
